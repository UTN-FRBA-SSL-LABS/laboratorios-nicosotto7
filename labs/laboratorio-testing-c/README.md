...
> R:carrito_init inicializa el carrito, generalmente poniendo la cantidad de productos en 0. Es fundamental llamarla porque, en C, las variables que no se inicializan arrancan con "basura" (valores aleatorios que quedaron en la memoria), lo que haría que el programa se rompa o calcule cualquier cosa.
...
> R:linea 24 y decia total += c->items[i].precio;

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R:Esa línea debía multiplicar por la cantidad para calcular bien el subtotal: total += c->items[i].precio * c->items[i].cantidad;.
...
> R:El test falla. El 5to carrito_agregar devuelve 1 (creyendo que lo agregó con éxito).
...
> R:El operador incorrecto es <= (menor o igual) y debería ser < (menor estricto).
...
> R:Sí, hay líneas con #####. Son las líneas 29 y 30 correspondientes a la función carrito_descuento. No se ejecutaron porque los tests unitarios (test_unitarios.c) no utilizan ni prueban esa función; recién la llamamos en los tests de integración.
...
> R:El test unitario prueba una sola función de forma aislada. El de integración verifica que varias funciones interactúen correctamente. El primer bug lo detectó el test unitario de la Parte 5.
...
> R:C no verifica automáticamente los límites de los arrays; si te pasás del tamaño, sobrescribís memoria que no te corresponde (buffer overflow). Lenguajes como Java o Python tienen protecciones que frenan el programa con una excepción antes de corromper la memoria.
...
> R:Los tests son automáticos y repetibles. Si mañana agregás una función nueva y rompés algo viejo por accidente, los tests te avisan en un segundo. Leer el código a ojo es muy propenso a errores humanos.
...
> R:El bug original era que el código sumaba solo el precio sin multiplicar por la cantidad. Como en el primer test la cantidad era 1, el cálculo erróneo devolvía el resultado numérico correcto por pura casualidad.
